...
def main(argv):
	...
	def trade(_buyExchange, _ask, _bid, _sellBalance, _buyBalance):
		...
		if (tradesize*_ask) > _buyBalance:
			newTradesize = _buyBalance / _ask*1.0
		...
		if (tradesize*_bid) > 0.0006001: # less than 10$
			...
			#Execute order
			if not args.dryrun:
				logger.info("Pas làààààààààààààààààààà")
				if _buyExchange == 0:
	...
	while True:
		...
		if not args.dryrun:
			...
			#print(" ======================= simulation ==================================")
			#print('\nASKS: Poloniex @ {:.8f} | {:.8f} @ Bitrex\nBIDS: Poloniex @ {:.8f} | {:.10f} @ Bitrex'.format(poloAsk, bittrexAsk, poloBid, bittrexBid))
			"""
			bittrexTargetBalance=500
			bittrexBaseBalance=0.01
			poloniexTargetBalance=500
			poloniexBaseBalance=0.01
			"""
